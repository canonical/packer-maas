...
build {
  ...
  post-processor "shell-local" {
    inline         = ["echo 'Syncing output-qemu/packer-qemu...'", "sync -f output-qemu/packer-qemu", "IMG_FMT=raw", "source ../scripts/setup-nbd", "TMP_DIR=$(mktemp -d /tmp/packer-maas-XXXX)", "echo 'Adding curtin-hooks to image...'", "mount $${nbd}p1 $TMP_DIR", "cp -r curtin $TMP_DIR", "sync -f $TMP_DIR/curtin", "umount $TMP_DIR", "echo 'Adding post-install scripts to image...'", "mount $${nbd}p6 $TMP_DIR", "cp -r maas $TMP_DIR", "python3 -m pip install -r requirements.txt --no-compile --target $TMP_DIR/maas", "find $TMP_DIR/maas -name __pycache__ -type d -or -name *.so | xargs rm -rf", "echo 'Unmounting image...'", "sync -f $TMP_DIR/maas", "umount $TMP_DIR", "qemu-nbd -d $nbd", "rmdir $TMP_DIR"]
